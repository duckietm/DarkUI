...
export const SelectReportedChatsView: FC<{}> = props =>
{
    ...
    return (
        <>
            <Column gap={ 1 }>
                <Text variant="white"  fontSize={ 4 }>{ LocalizeText('help.emergency.chat_report.subtitle') }</Text>
                <Text variant="white" >{ LocalizeText('help.emergency.chat_report.description') }</Text>
            </Column>
            <Column gap={ 1 } overflow="hidden">
                { !userChats || !userChats.length &&
                    <Text variant="white" >{ LocalizeText('help.cfh.error.no_user_data') }</Text> }
                { (userChats.length > 0) &&
                        ...
                        { userChats.map((chat, index) =>
                        {
                            return (
                                <LayoutGridItem key={ chat.id } onClick={ event => selectChat(chat) } itemActive={ (selectedChats.indexOf(chat) >= 0) }>
                                    <Text variant="white" >{ chat.message }</Text>
                                </LayoutGridItem>
                            );
                        }) }
                    </AutoGrid> }
