        ...
        <NitroCardView uniqueKey="achievements" className="nitro-achievements">
            ...
            { selectedCategory &&
                <NitroCardSubHeaderView position="relative" className="justify-content-center align-items-center cursor-pointer" gap={ 3 }>
                    ...
                    <Column grow gap={ 0 }>
                        <Text variant="white"  fontSize={ 4 } fontWeight="bold" className="text-small">{ LocalizeText(`quests.${ selectedCategory.code }.name`) }</Text>
                        <Text variant="white" >{ LocalizeText('achievements.details.categoryprogress', [ 'progress', 'limit' ], [ selectedCategory.getProgress().toString(), selectedCategory.getMaxProgress().toString() ]) }</Text>
                    </Column>
                ...
                { !selectedCategory &&
                    <>
                        ...
                        <Column grow justifyContent="end" gap={ 1 }>
                            <Text variant="white"  small center>{ LocalizeText('achievements.categories.score', [ 'score' ], [ achievementScore.toString() ]) }</Text>
                            <LayoutProgressBar style={{fontSize: "13px"}} text={`${getProgress.toString()}/${getMaxProgress.toString()}`} progress={ getProgress } maxProgress={ getMaxProgress } />
                        </Column>
                    </> }
                { selectedCategory &&
