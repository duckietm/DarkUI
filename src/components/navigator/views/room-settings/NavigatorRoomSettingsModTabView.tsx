...
export const NavigatorRoomSettingsModTabView: FC<NavigatorRoomSettingsTabViewProps> = props =>
{
    ...
    return (
        <Grid overflow="auto">
            <Column size={ 6 }>
                <Text variant="white" bold>{ LocalizeText('navigator.roomsettings.moderation.banned.users') } ({ bannedUsers.length })</Text>
                        ...
                        { bannedUsers && (bannedUsers.length > 0) && bannedUsers.map((user, index) =>
                        {
                            return (
                                <Flex key={ index } shrink alignItems="center" gap={ 1 } overflow="hidden">
                                    ...
                                    <Text variant="white" pointer grow onClick={ event => setSelectedUserId(user.userId) }> { user.userName }</Text>
                                </Flex>
                            );
                        }) }
                    </Column>
            ...
            <Column size={ 6 }>
                <Column gap={ 1 }>
                    <Text variant="white" bold>{ LocalizeText('navigator.roomsettings.moderation.mute.header') }</Text>
                    <Flex alignItems="center" gap={ 1 }>
                        ...
                        <Text variant="white">{ LocalizeText('navigator.roomsettings.moderation.rights') }</Text>
                    </Flex>
                </Column>
                <Column gap={ 1 }>
                    <Text variant="white" bold>{ LocalizeText('navigator.roomsettings.moderation.kick.header') }</Text>
                    <Flex alignItems="center" gap={ 1 }>
                        ...
                        <Text variant="white">{ LocalizeText('navigator.roomsettings.moderation.all') }</Text>
                    </Flex>
                    <Flex alignItems="center" gap={ 1 }>
                ...
                <Column gap={ 1 }>
                    <Text variant="white" bold>{ LocalizeText('navigator.roomsettings.moderation.ban.header') }</Text>
                    <Flex alignItems="center" gap={ 1 }>
