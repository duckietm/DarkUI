...
export const CatalogLayoutMarketplaceItemView: FC<MarketplaceItemViewProps> = props =>
{
    ...
    return (
        <LayoutGridItem shrink center={ false } column={ false } alignItems="center" className="p-1">
            ...
            <Column grow gap={ 0 }>
                <Text variant="white" fontWeight="bold">{ getMarketplaceOfferTitle }</Text>
                { (type === OWN_OFFER) &&
                    <>
                        <Text variant="white">{ LocalizeText('catalog.marketplace.offer.price_own_item', [ 'price' ], [ offerData.price.toString() ]) }</Text>
                        <Text variant="white">{ offerTime() }</Text>
                    </> }
                { (type === PUBLIC_OFFER) &&
                    <>
                        <Text variant="white">{ LocalizeText('catalog.marketplace.offer.price_public_item', [ 'price', 'average' ], [ offerData.price.toString(), ((offerData.averagePrice > 0) ? offerData.averagePrice.toString() : '-') ]) }</Text>
                        <Text variant="white">{ LocalizeText('catalog.marketplace.offer_count', [ 'count' ], [ offerData.offerCount.toString() ]) }</Text>
                    </> }
            </Column>
            <Column gap={ 1 }>
